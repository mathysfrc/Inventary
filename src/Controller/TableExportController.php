...
class TableExportController extends AbstractController
{
    ...
    public function exportTable(EntityManagerInterface $entityManager): Response
    
        {
            ...
            $txtContent = "SKU\tDescription\tDimension\tRésultat\tPrix\tFamille produit\tRéférence\tStatus\n"; 
            ...
            foreach ($donnees as $item) {
                ...
                $txtContent .= "\t" . $item->getPrice() .  ' € '  .  "\t" . $item->getProductFamily() .  "\t" . $item->getReference() . "\t" . $item->getStatus() . "\n";
            }
